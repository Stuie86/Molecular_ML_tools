...
integer 			::	O_lap,o_lapx,o_lapy,o_lapz,lap_val(3)
parameter                               (b2a=0.529177249,pi=3.14159265359)
...
if(configNo.eq.0) then							!
  print*,'No configurations counted. Check TS format and file name';stop!
endif									!
rewind(1)								!
   ...
   do config=1,configNo												!grid cell for each water, and add to array
      do j=1,NoWaters												!
         ...
         z_atom=nint(cartesians(config,j,3))-z_min+1	!1 to x_max-x_min+1					!
	 													!			
         do o_lapx=-o_lap,o_lap;do o_lapy=-o_lap,o_lap;do o_lapz=-o_lap,o_lap					!
